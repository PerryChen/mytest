  ...
  scripts: {
    ...
    2: {
      ...
      "find_bug": {
        speaker: "小唯",
        avatar: "🧑",
        text: "等等，我发现这电池仓盖好像有点干涉，很难扣上。这要紧吗？",
        next: "eng_response"
      },
      "eng_response": {
        speaker: "工程师",
    ...
    3: {
      ...
      "strict_pass": {
        speaker: "小唯",
        avatar: "🧑",
        text: "好的，记录在清单里，量产前必须解决。现在，启动模具！",
        next: "certification"
      },
      "certification": {
        speaker: "认证专员",
    ...
    6: {
      "start": {
        speaker: "仓储 Jason",
        avatar: "👷♂️",
        text: "Hi V! 这里是美西仓库。你的 Discover 3 刚刚清关到达，正在卸货。",
        next: "check_stock"
      },
      "check_stock": {
        speaker: "小唯",
  ...
  init() {
    ...
    this.popup.closeBtn.addEventListener('click', () => {
      ...
      if (Game.pendingDialogueNode) {
        ...
        Game.advanceDialogue(); // 没有挂起的对话，继续剧情
      }
    });
...
const Game = {
  ...
  pendingDialogueNode: null, // 知识卡显示时挂起的对话节点
  ...
  playDialogue(nodeId) {
    ...
    // 检查是否解锁知识卡
    // NOTE: 必须在渲染对话之前检查，确保知识卡先显示，关闭后再显示对话
    // 这样用户不会忽略底部的对话内容
    if (node.unlockCard) {
      if (GameState.unlockCard(node.unlockCard)) {
        // 如果是新解锁，显示弹窗，并保存当前节点以便关闭弹窗后渲染对话
